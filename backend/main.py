...
import threading
from database import get_db, Company, News, init_db, SessionLocal
            ...
            if not company:
                ...
                if ticker and (ticker.endswith(".NS") or ticker.endswith(".BO")):
                    ...
                    if existing_company:
                        ...
                        print(
                            f"Discovered new Indian company: {company_name} ({ticker})"
                        )
                        company = Company(
                            name=company_name, ticker=ticker, sector="Discovered"
                        )
                        db.add(company)
                        try:
                            db.commit()
...
async def trigger_analysis(
    ...
    background_tasks.add_task(
        process_company_news, company.ticker, company.id, company.name, db
    )
