    ...
    def get_sentiment(self, text: str) -> Tuple[str, float]:
        ...
        try:
            for attempt in range(3):
                try:
                    results = self.client.text_classification(
                        clean_text, model=self.SENTIMENT_MODEL
                    )
                        ...
                        if label == "neutral" and score < 0.6:
                            ...
                            if len(sorted_results) > 1:
                                ...
                                if second_best.score > 0.3:
                                    if second_best.label == "positive":
                                        return "Positive", float(second_best.score)
                                    if second_best.label == "negative":
                                        return "Negative", float(second_best.score)
